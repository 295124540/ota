...
class Dynamic extends ActiveController
{
    ...
    protected function prepareDataProvider()
    {
        ...
        foreach ($list as $m){
            ...
            $m->user;
            $m['user_name'] = $m['user']['nickname'];
            $m['user_head_img']= $m['user']['head_img_url'];
            //unset($m['user']);
    ...
    public function comment(){
        ...
        if(!DynamicModel::get($params['dynamic_id']))error("该动态不存在");
        $params['user_id']=$this->userId;
        $params['user_name']=$this->user['nickname'];
        $params['head_img']=$this->user['head_img_url'];
        $comment = new DynamicComment($params);
    ...
    public function commentReply(){
        $params = Request::instance()->param();
        $rules = [
            'comment_id'=>'require|max:30',
            'to_user_name'=>'require',
            'reply'=>'require|max:100',
        ];
        $this->validate($params,$rules);
        if(!DynamicComment::get($params['comment_id']))error("该评论不存在！");
        $params['user_name'] = $this->user['nickname'];
        $params['create_time'] = time();
        $model = new DynamicCommentReply($params);
